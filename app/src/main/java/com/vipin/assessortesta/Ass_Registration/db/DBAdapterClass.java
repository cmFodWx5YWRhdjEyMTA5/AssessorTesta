package com.vipin.assessortesta.Ass_Registration.db;
...
public class DBAdapterClass {
    ...
    // --- QUERY BELOW
    public void insertAcademicData(String qualification, String college_name, String joining_year, String final_year)
    {
        try
        {
            ContentValues cv = new ContentValues();
            cv.put("qualification", qualification);
            cv.put("college_name", college_name);
            cv.put("joining_year", joining_year);
            cv.put("final_year", final_year);

            mDb.insert("tblAcademicInfo", null, cv);
        }catch (SQLException se) {
            Log.e(TAG, "#Error :" + se, se);
            Toast.makeText(mContext, "Academic data insertion failed", Toast.LENGTH_LONG).show();
        }
    }


    public void deleteAcademicTable()
    {
        try
        {
            mDb.delete("tblAcademicInfo",null,null);
        }catch (SQLException se)
        {
            Log.e(TAG, "Error : "+se,se);
        }
    ...
    public ArrayList<AcademicDbModel> retrieveAcademicData()
    {
        ArrayList<AcademicDbModel> arrayListProjectUpdate = new ArrayList<>();

        try {
            String sql = "SELECT * FROM tblAcademicInfo";
            Cursor mCur = mDb.rawQuery(sql, null);
            if (mCur != null) {
                mCur.moveToNext();
            }
            while (mCur.isAfterLast() == false) {
                String qualification = mCur.getString(mCur.getColumnIndex("qualification"));
                String college_name = mCur.getString(mCur.getColumnIndex("college_name"));
                String joining_year = mCur.getString(mCur.getColumnIndex("joining_year"));
                String final_year = mCur.getString(mCur.getColumnIndex("final_year"));


                AcademicDbModel academicDbModel = new AcademicDbModel(qualification, college_name, joining_year, final_year);
                arrayListProjectUpdate.add(academicDbModel);
                mCur.moveToNext();
            }
        }catch (SQLException se)
        {
...
// ----------------------------------------------JOB
    ...
    // --- QUERY BELOW
    public void insertJobData(String jobName, String companyName, String joining_year, String final_year)
    {
        try
        {
            ContentValues cv = new ContentValues();
            cv.put("position", jobName);
            cv.put("company_name", companyName);
            cv.put("joining_year", joining_year);
            cv.put("final_year", final_year);

            mDb.insert("tblJobInfo", null, cv);
        }catch (SQLException se) {
            Log.e(TAG, "#Error :" + se, se);
            Toast.makeText(mContext, "job data insertion failed", Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void deleteJobTable()
    {
        try
        {
            mDb.delete("tblJobInfo",null,null);
        }catch (SQLException se)
        {
            Log.e(TAG, "Error : "+se,se);
        }
    ...
    public ArrayList<AcademicDbModel> retrieveJobData()
    {
        ArrayList<AcademicDbModel> arrayListProjectUpdate = new ArrayList<>();

        try {
            String sql = "SELECT * FROM tblJobInfo";
            Cursor mCur = mDb.rawQuery(sql, null);
            if (mCur != null) {
                mCur.moveToNext();
            }
            while (mCur.isAfterLast() == false) {
                String qualification = mCur.getString(mCur.getColumnIndex("position"));
                String college_name = mCur.getString(mCur.getColumnIndex("company_name"));
                String joining_year = mCur.getString(mCur.getColumnIndex("joining_year"));
                String final_year = mCur.getString(mCur.getColumnIndex("final_year"));
...
//    public ArrayList<ProjectUpdatePojo> getProjectU+pdateData()
//    {
